...
int user_send_req() {
    ...
    struct flow_key* buf = malloc(buf_size);
    bzero(buf, buf_size);
    write(sockfd, buf, buf_size);

    elemtype* recv_buf = malloc(MAX_BATCH * sizeof(elemtype));
    bzero(recv_buf, MAX_BATCH * sizeof(elemtype));
    int n = read(sockfd, recv_buf, MAX_BATCH * sizeof(elemtype));
    printf("get resonse!\n");
    for (int i = 0; i < n / sizeof(elemtype); ++i) {
        printf("%llu ", recv_buf[i]);
    }
    return 0;
}
