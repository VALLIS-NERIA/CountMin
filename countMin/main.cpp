...
int main(int argc, char** argv) {
	...
	while (true) {
		...
		case NLREQUEST_FETCH:
			traffic = cm.query(key, key_length);
			reply_packet reply;
			reply.countmin_traffic = traffic;
			reply.flow = spkt.flow_pointer;
			reply.key = spkt.key;
            length = sizeof(reply);
            printf("flow : %s   cm : %llu   \n", spkt.key.to_string().c_str(), traffic);
		...
		case NLREQUEST_REPORT:
            if (spkt.packet_size) {
                ...
                cm.add(sw_name, key, key_length, spkt.packet_size);
                traffic = cm.query(key, key_length);                
